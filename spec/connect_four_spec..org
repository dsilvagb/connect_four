...
describe Player do
  ...
  describe '#update_player_name' do
    context 'when updating player name' do
      subject(:player_update) { described_class.new('player', 'R', '@board') }

      it 'to Tommy' do
        result = player_update.update_player_name('Tommy')
        expect(player_update.name).to eq(result)
      end
    end
  end
end
...
describe Board do
  ...
  describe '#initialize' do
    context 'when board is initalize' do
  ...
  describe '#number_input_range' do
    context 'when number input is out of range' do
      before do
        ...
        allow(board).to receive(:number_input_range).with('Test', 1, 7).and_return(msg)
      end
